...
// The lightweight Flask backend exposes polling rather than a WebSocket.
async function pollLegacyTelemetry() {
    const data = await apiGet("/api/dashboard-data");
    const vitals = data && data.active_vitals;
    if (!vitals || !state.selectedPatientId) return;
    if (Array.isArray(vitals.ecg)) {
        const queue = state.ecgQueues[state.selectedPatientId] || [];
        queue.push(...vitals.ecg);
        state.ecgQueues[state.selectedPatientId] = queue.slice(-500);
    }
    const patient = state.patients[state.selectedPatientId];
    if (patient) {
...
async function initApp() {
    ...
    setInterval(pollLegacyTelemetry, 1000);
